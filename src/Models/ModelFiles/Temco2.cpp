...
#define U_THERMAL(u,n)        (UNKNOWN(u,n,E_THERMAL))    /*added*/
...
struct ImplicitValues_t {
  ...
  /*added thermal*/
  T Entropy_total;
  T HeatFlow[Element_MaxNbOfNodes];
} ;
...
/*added*/
/* Melting temperature of ice at atmospheric pressure, reference temperature */
static double T_m;
/* Atmospheric pressure (Pa) , reference pressure*/
static double p_m;
/* Specific heat of liquid water (J/kg/K) */
static double C_l;
/* Specific heat of liquid gas (J/kg/K) */
static double C_g;
/* SolidVolumetricHeat (J/m3/K) */
static double C_s;
/* Thermal conductivity of solid (W/m/K) */
#define LAM_s       ((1.0) * Watt / m1 / Kelvin)
static double lam_s;
/* Thermal conductivity of liquid water (W/m/K) */
#define LAM_l       ((0.6) * Watt / m1 / Kelvin)
static double lam_l;
/* Thermal conductivity of gas (W/m/K) */
#define LAM_g       ((2.2) * Watt / m1 / Kelvin)         /*需要修改*/
static double lam_g;
...
/* Thermal volumetric expansion coefficient of gas (1/K) */
#define ALPHA_g           ((155.e-6) * (1./Kelvin))   /*需要修改*/
static double alpha_g;
...
/* Melting temperature of ice at atmospheric pressure, reference temperature */
  ...
  p_m = (1.e5) * Pa;
  /*SolidVolumetricHeat (J/m3/K) */
  C_s = (2e+06) * Joule / m3 / Kelvin;
  /* Specific heat of liquid water (J/kg/K) */
  C_l = (4180.) * Joule / kg / Kelvin;
  /* Specific heat of gas (J/kg/K) */
  C_g = (1) * Joule / kg / Kelvin;              /*需要修改*/
  /* Thermal conductivity of liquid water (W/m/K) */
  lam_l = LAM_l;
  /* Thermal conductivity of gas (W/m/K) */
  lam_g = LAM_g;
...
int SetModelProp(Model_t* model)
{
 ...
 /*added*/ 
#ifdef E_THERMAL
 Model_CopyNameOfEquation(model, E_THERMAL, "thermal");
#endif
...
/*added*/  
#ifdef E_THERMAL
  Model_CopyNameOfUnknown(model,E_THERMAL, "temp") ;
#endif
...
/* Lecture des donnees materiaux dans le fichier ficd */
{
  ...
  /*added*/
 double Initial_temperature = 298;
 ComputePhysicoChemicalProperties(Initial_temperature);

  {
    if(!csd) csd = CementSolutionDiffusion_Create() ;
    if(!hcc_d) hcc_d = HardenedCementChemistry_Create<double>() ;
    HardenedCementChemistry_SetRoomTemperature(hcc_d,Initial_temperature) ;
    
    #ifdef HAVE_AUTODIFF
    if(!hcc_r) hcc_r = HardenedCementChemistry_Create<real>() ;
    HardenedCementChemistry_SetRoomTemperature(hcc_r,Initial_temperature) ;
    #endif
    
    CementSolutionDiffusion_SetRoomTemperature(csd,Initial_temperature) ;
  }
...
int  ComputeResidu(Element_t* el,double t,double dt,double* r)
{
  ...
  #ifdef E_AIR
  {
    ...
    /* 11.  Conservation of entropy: (M_Entropy - M_Entropyn) + dt * div(W_Heat) = 0 */
#ifdef E_THERMAL
    {
        int imass = Values_Index(Entropy_total);
        int iflow = Values_Index(HeatFlow[0]);
        mpm.ComputeMassConservationResiduByFVM(el, t, dt, r, E_THERMAL, imass, iflow);
    }
#endif
...
#define ptZ_CSH    &(HardenedCementChemistry_GetWaterSiliconRatioInCSH(hcc))
  ...
  Result_Store(r + i++, &val.Entropy_total, "total entropy", 1);
    ...
    /* Mass flows */
    {
      ...
      Result_Store(r + i++, vi[0].HeatFlow + 1, "heatflow", 1);  /*output heatflow is added*/
    }
    ...
    {  /*added*/
      double temp = TEMP(u,j) ;
      
      Result_Store(r + i++,&temp,"Temperature",1) ;
    }
  ...
  {        
    for(int j = 0 ; j < nn ; j++) {
      ...
      if(j == i) {
        ...
        #endif
        #ifdef E_THERMAL
        cij[E_THERMAL*NEQ       + k] = dval.Entropy_total ;
        #endif            /*added*/
      }
      ...
      if(j != i) {        
        ...
        #endif
        #ifdef E_THERMAL
        cij[E_THERMAL*NEQ       + k] = - dtdij*dval.HeatFlow[j] ;
        #endif /*added*/
      }
    }
  }
...
void  MPM_t::SetIndexOfPrimaryVariables(Element_t* el,int* ind)
{
  ...
  /*added*/
  #ifdef E_THERMAL 
  ind[E_THERMAL] = Values_Index(U_thermal);
  #endif
}
...
void MPM_t::SetIncrementOfPrimaryVariables(Element_t* el,double* dui)
{
  ...
  int nn = Element_GetNbOfNodes(el);
    
    {
      ...
      /*added*/
      #ifdef E_THERMAL      
      dui[E_THERMAL      ] =  1.e-3 * ObVal_GetValue(obval + E_THERMAL) ;
      #endif
    }
}
...
/** Compute the secondary variables from the primary ones. */
{
  ...
  /*added temperature*/
  T temp       = val.U_thermal ;
  
  HardenedCementChemistry_t<T>* hcc = hcc_func<T>();
   /* UPDATED properties? needed?*/
 /* 根据当前温度重新计算物理化学性质 */
 ComputePhysicoChemicalProperties(temp);
 
 /* 更新化学模块的温度 */
 HardenedCementChemistry_SetRoomTemperature(hcc, temp);
 CementSolutionDiffusion_SetRoomTemperature(csd, temp);
  ...
  /* Solve cement chemistry */
  {
    ...
    #ifdef E_CHLORINE
    HardenedCementChemistry_ComputeSystem(hcc,CaO_SiO2_Na2O_K2O_CO2_Cl_H2O,0) ;
    #else
    HardenedCementChemistry_SetAqueousConcentrationOf(hcc,Cl,c_cl) ;
    HardenedCementChemistry_ComputeSystem(hcc,CaO_SiO2_Na2O_K2O_CO2_H2O,0) ;
    #endif
  ...
  /*added*/
    /*Entropies added*/
  T alpha_l = ALPHA_l(temp - T_m);
    T s_liquid   = C_l*log(temp/T_m) - alpha_l/rho_l0 *(p_l - p_m) ;
    //double s_g   = C_g*log(tem/T_m) - alpha_g/rho_g0 *(p_g - p_m) ;
    T s_sol = C_s*log(temp/T_m) ;
    //double s_tot = s_sol + m_l*s_liquid + m_g*s_gas;
    T s_tot = s_sol + m_l*s_liquid;
  ...
  */
  {
    ...
    {
     /* Transfer coefficients--entropies : s_liquid :已经计算完 and ThermalConductivity*/
   T lam_1 = s_l*lam_l + s_g*lam_g - lam_s ;
   T lam_2 = lam_s / lam_1 ;
   T phi_lam = (1-phi)/3 ;
   T lam_hom = lam_s*(1 + phi/(phi_lam+lam_2)) ;
   T kth     = lam_hom/temp ;
...
Values_d*  MPM_t::SetFluxes(Element_t* el,double const& t,int const& i,int const& j,Values_d const& grdval,Values_d* val)
{
  ...
  double grd_p_l = grdval.Pressure_liquid ;
    double kd_l    = valij.Permeability_liquid ;
    double w_l     = - kd_l * grd_p_l  ;
  double kth  = valij.ThermalConductivity ;    /*  lam_hom/T */
    double entropy_l = valij.Entropy_liquid ;
  double grd_temp = grdval.U_thermal ;
  vali.HeatFlow[j] = entropy_l*w_l - kth*grd_temp;
  }
  ...
  
  {
    ...
    valj.HeatFlow[i]= -vali.HeatFlow[j];   /*added*/
  }
